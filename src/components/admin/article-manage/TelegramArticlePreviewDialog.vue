  ...
  >
    <div v-if="article" class="telegram-message-preview">
      ...
      <div v-if="article.status === 'APPROVED'" class="telegram-actions">
        <el-button type="primary" size="large" @click="$emit('publish', article); visible = false">
          发布到频道
        </el-button>
      </div>
      <div v-else-if="article.status === 'PENDING'" class="telegram-actions">
        <el-button type="primary" size="large" @click="$emit('review', article); visible = false">
          去审核
        </el-button>
      </div>
    </div>
